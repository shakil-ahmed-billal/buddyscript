import React from "react";

export interface IconProps extends React.SVGProps<SVGSVGElement> {
  name: "home" | "explore" | "videos" | "photos" | "friends" | "bookmarks" | "gaming" | "settings" | "save" | "bell" | "chat" | "search" | "chevron-down" | "more-vertical" | "photo-fill" | "video-fill" | "event-fill" | "article-fill" | "send-fill" | "like" | "comment" | "share" | "globe" | "help" | "logout" | "close" | "plus" | "feed" | "event" | "watch" | "files" | "marketplace" | "artical" | "haha" | "insights";
  size?: number;
  active?: boolean;
}

export const BsIcon = ({ name, size = 20, active = false, className = "", ...props }: IconProps) => {
  const color = active ? "#1890FF" : "currentColor";
  const opacity = active ? 1 : 0.6;

  switch (name) {
    case "insights":
      return (
        <svg width={size} height={size} viewBox="0 0 22 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path fill={color} fillOpacity={opacity} d="M14.96 2c3.101 0 5.159 2.417 5.159 5.893v8.214c0 3.476-2.058 5.893-5.16 5.893H6.989c-3.101 0-5.159-2.417-5.159-5.893V7.893C1.83 4.42 3.892 2 6.988 2h7.972zm0 1.395H6.988c-2.37 0-3.883 1.774-3.883 4.498v8.214c0 2.727 1.507 4.498 3.883 4.498h7.972c2.375 0 3.883-1.77 3.883-4.498V7.893c0-2.727-1.508-4.498-3.883-4.498zM7.036 9.63c.323 0 .59.263.633.604l.005.094v6.382c0 .385-.285.697-.638.697-.323 0-.59-.262-.632-.603l-.006-.094v-6.382c0-.385.286-.697.638-.697zm3.97-3.053c.323 0 .59.262.632.603l.006.095v9.435c0 .385-.285.697-.638.697-.323 0-.59-.262-.632-.603l-.006-.094V7.274c0-.386.286-.698.638-.698zm3.905 6.426c.323 0 .59.262.632.603l.006.094v3.01c0 .385-.285.697-.638.697-.323 0-.59-.262-.632-.603l-.006-.094v-3.01c0-.385.286-.697.638-.697z" />
        </svg>
      );
    case "haha":
      return (
        <svg width={size} height={size} viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M9.5 19a9.5 9.5 0 100-19 9.5 9.5 0 000 19z" fill="#FFCC4D" />
          <path d="M9.5 11.083c-1.912 0-3.181-.222-4.75-.527-.358-.07-1.056 0-1.056 1.055 0 2.111 2.425 4.75 5.806 4.75 3.38 0 5.805-2.639 5.805-4.75 0-1.055-.697-1.125-1.055-1.055-1.57.305-2.838.527-4.75.527z" fill="#664500" />
          <path d="M4.75 11.611s1.583.528 4.75.528 4.75-.528 4.75-.528-1.056 2.111-4.75 2.111-4.75-2.11-4.75-2.11z" fill="#fff" />
          <path d="M6.333 8.972c.729 0 1.32-.827 1.32-1.847s-.591-1.847-1.32-1.847c-.729 0-1.32.827-1.32 1.847s.591 1.847 1.32 1.847zM12.667 8.972c.729 0 1.32-.827 1.32-1.847s-.591-1.847-1.32-1.847c-.729 0-1.32.827-1.32 1.847s.591 1.847 1.32 1.847z" fill="#664500" />
        </svg>
      );
    case "feed":
      return (
        <svg width={size} height={size} viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M11.667 18.333H8.333c-5.833 0-7.5-1.666-7.5-7.5v-1.666c0-5.834 1.667-7.5 7.5-7.5h3.334c5.833 0 7.5 1.666 7.5 7.5v1.666c0 5.834-1.667 7.5-7.5 7.5zm-3.334-15c-4.933 0-6.111.958-6.111 6.111v1.666c0 5.153 1.178 6.111 6.111 6.111h3.334c4.933 0 6.111-1.178 6.111-6.111v-1.666c0-4.934-1.178-6.111-6.111-6.111H8.333zm-4.325 5h11.984c.384 0 .695.31.695.694 0 .384-.311.695-.695.695H4.008c-.384 0-.694-.311-.694-.695 0-.383.31-.694.694-.694zm0 3.334h11.984c.384 0 .695.31.695.694 0 .384-.311.695-.695.695H4.008c-.384 0-.694-.311-.694-.695 0-.383.31-.694.694-.694z" fill={color} />
        </svg>
      );
    case "event":
      return (
        <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M16.143 2c.621 0 1.134.46 1.192 1.056l.006.091c0 .034 0 .07 0 .101.996.098 1.785.49 2.35 1.144.576.666.863 1.58.863 2.72v10.518c0 1.14-.287 2.054-.863 2.719-.575.666-1.385 1.066-2.4 1.157l-.004.094a1.2 1.2 0 01-2.4 0l-.004-.094H9.11l-.004.094a1.2 1.2 0 01-2.4 0l-.004-.094c-.958-.087-1.748-.466-2.32-.1-.575.665-.863 1.58-.863 2.72V7.112c0-1.14.288-2.054.863-2.72.565-.653 1.354-1.046 2.35-1.144 0-.03 0-.067 0-.101 0-.634.537-1.147 1.2-1.147.62 0 1.133.459 1.191 1.056l.007.091 0 .114h7.022V3.247c0-.633.538-1.147 1.2-1.147zM19.011 9.8H4.992v7.83c0 .736.166 1.272.474 1.626s.804.551 1.503.58l.142.002H16.89c.772 0 1.294-.22 1.624-.582.32-.35-.3-.902-.3-1.626V9.8zm-2.868-6.353c-.22 0-.4.162-.4.362v1.547c0 .2.18.361.4.361.22 0 .4-.162.4-.361V3.81c0-.2-.18-.363-.4-.363zM7.86 3.447c-.221 0-.4.162-.4.363v1.547c0 .2.179.361.4.361.22 0 .4-.162.4-.361V3.81c0-.2-.18-.363-.4-.363z" fill={color} />
        </svg>
      );
    case "watch":
      return (
        <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M12.015 2c5.441 0 9.851 4.477 9.851 10 0 5.523-4.41 10-9.851 10s-9.85-4.477-9.85-10c0-5.523 4.41-10 9.85-10zm0 1.448c-4.636 0-8.396 3.829-8.396 8.552s3.76 8.552 8.396 8.552 8.396-3.829 8.396-8.552-3.76-8.552-8.396-8.552zm3.328 8.163l.113.067.112.073c.48.337.48.86 0 1.196-.06.042-.14.092-.225.14l-4.526 2.656a1.144 1.144 0 01-1.745-.989v-5.215a1.14 1.14 0 011.758-.98l4.513 3.052z" fill={color} />
        </svg>
      );
    case "files":
      return (
        <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M12.914 2.115l.135.008c.553.031.785.163 1.15.52l.094.1h-.001l5.12 5.378c.414.436.561.745.584 1.349l.004.148v7.834c0 1.02-.279 1.836-.838 2.428-.56.592-1.348.95-2.333 1.031l-.101.09a.12.12 0 01-2.4 0l-.004-.09H9.426l-.004.09a.12.12 0 01-2.4 0l-.004-.09c-1.026-.08-1.889-.452-2.483-1.074-.575-.592-.857-1.408-.857-2.428V6.166c0-1.019.282-1.835.857-2.428C6.13 3.118 6.993 2.748 8.02 2.668l.004-.09a.12.12 0 012.4 0l.004.09H12.91zm5.12 7.151h-3.486c-.733 0-1.222.48-1.222 1.201V13.8c0 .72.489 1.2 1.222 1.2h3.486c.732 0 1.221-.48 1.221-1.2v-3.333c0-.72-.489-1.201-1.221-1.201zM11.667 3.535H8.02c-.771 0-1.294.221-1.623.582-.32.35-.474.887-.474 1.626l.001 1.701V12.148c0 1.151.282 2.073.864 2.745l.1.114a.12.12 0 01-1.2 0l.004-.11c-.575-.666-.864-1.583-.864-2.731V10.45l.001-1.7V5.743c0-1.148.289-2.065.864-2.73s1.393-1.047 2.378-1.13h3.626c.22 0 .4.162.4.362s-.18.362-.4.362H8.02c-.22 0-.4-.162-.4-.362zm4.312 4.957l.001-3.61c0-.4-.344-.644-.7-.644l.01-.11a.114.114 0 011.147 1.147v3.088l-.458.129z" fill={color} />
        </svg>
      );
    case "marketplace":
      return (
        <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M12 2c5.441 0 9.851 4.477 9.851 10 0 5.523-4.41 10-9.851 10s-9.85-4.477-9.85-10c0-5.523 4.41-10 9.85-10zm0 1.448c-4.636 0-8.396 3.829-8.396 8.552s3.76 8.552 8.396 8.552 8.396-3.829 8.396-8.552-3.76-8.552-8.396-8.552zm3.328 8.163l.113.067.112.073c.48.337.48.86 0 1.196-.06.042-.14.092-.225.14l-4.526 2.656a1.144 1.144 0 01-1.745-.989v-5.215a1.14 1.14 0 011.758-.98l4.513 3.052z" fill={color} />
        </svg>
      );
    case "artical":
      return (
        <svg width={size} height={size} viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M15.908 2c.671 0 1.315.267 1.784.74.468.473.72 1.112.699 1.709l.009.14a.985.985 0 00.136.395c.145.242.382.418.659.488.277.071.57.03.849-.13l.156-.078c1.164-.538 2.563-.11 3.209.991l.581.99a.695.695 0 01.04.081l.054.107c.52 1.089.15 2.385-.838 3.043l-.243.15a1.046 1.046 0 00-.313.339 1.041 1.041 0 00-.11.805c.074.272.254.504.53.66l.157.1c.478.328.824.812.973 1.367.171.626.08 1.292-.257 1.86l-.624 1.022-.095.144c-.734 1.038-2.159 1.355-3.247.738l-.13-.066a1.122 1.122 0 00-.411-.095 1.087 1.087 0 00-.766.31c-.204.2-.317.471-.316.786l-.008.163C16.279 18.022 15.204 19 13.908 19h-1.17c-1.373 0-2.486-1.093-2.484-2.398l-.008-.14a.994.994 0 00-.14-.401 1.066 1.066 0 00-.652-.493 1.12 1.12 0 00-.853.127l-.168.083a2.527 2.527 0 01-1.699.122 2.47 2.47 0 01-1.487-1.154l-.604-1.024-.081-.152a2.404 2.404 0 01.975-3.132l.1-.061c.292-.199.467-.527.467-.877 0-.381-.207-.733-.568-.94l-.147-.092a2.42 2.42 0 01-.725-3.236l.615-.993a2.502 2.502 0 013.366-.912l.126.066c.13.058.269.089.403.09a1.08 1.08 0 001.085-1.068l.009-.185c.048-.57.3-1.106.713-1.513A2.5 2.5 0 0112.738 0h1.17zM13.908 6.396a3.105 3.105 0 100 6.21 3.105 3.105 0 000-6.21z" fill={color} />
        </svg>
      );
    case "home":
      return (
        <svg width={size} height={size} viewBox="0 0 18 21" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M1 9.924c0-1.552 0-2.328.314-3.01.313-.682.902-1.187 2.08-2.196l1.143-.98C6.667 1.913 7.732 1 9 1c1.268 0 2.333.913 4.463 2.738l1.142.98c1.179 1.01 1.768 1.514 2.081 2.196.314.682.314 1.458.314 3.01v4.846c0 2.155 0 3.233-.67 3.902-.669.67-1.746.67-3.901.67H5.57c-2.155 0-3.232 0-3.902-.67C1 18.002 1 16.925 1 14.77V9.924z" stroke={color} strokeOpacity={opacity} strokeWidth="1.5" />
          <path d="M11.857 19.341v-5.857a1 1 0 00-1-1H7.143a1 1 0 00-1 1v5.857" stroke={color} strokeOpacity={opacity} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      );
    case "friends":
      return (
        <svg width={size} height={size} viewBox="0 0 26 20" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path fillRule="evenodd" clipRule="evenodd" d="M12.79 12.15h.429c2.268.015 7.45.243 7.45 3.732 0 3.466-5.002 3.692-7.415 3.707h-.894c-2.268-.015-7.452-.243-7.452-3.727 0-3.47 5.184-3.697 7.452-3.711l.297-.001h.132zm0 1.75c-2.792 0-6.12.34-6.12 1.962 0 1.585 3.13 1.955 5.864 1.976l.255.002c2.792 0 6.118-.34 6.118-1.958 0-1.638-3.326-1.982-6.118-1.982zm9.343-2.224c2.846.424 3.444 1.751 3.444 2.79 0 .636-.251 1.794-1.931 2.43a.882.882 0 01-1.137-.506.873.873 0 01.51-1.13c.796-.3.796-.633.796-.793 0-.511-.654-.868-1.944-1.06a.878.878 0 01-.741-.996.886.886 0 011.003-.735zm-17.685.735a.878.878 0 01-.742.997c-1.29.19-1.944.548-1.944 1.059 0 .16 0 .491.798.793a.873.873 0 01-.314 1.693.897.897 0 01-.313-.057C.25 16.259 0 15.1 0 14.466c0-1.037.598-2.366 3.446-2.79.485-.06.929.257 1.002.735zM12.789 0c2.96 0 5.368 2.392 5.368 5.33 0 2.94-2.407 5.331-5.368 5.331h-.031a5.329 5.329 0 01-3.782-1.57 5.253 5.253 0 01-1.553-3.764C7.423 2.392 9.83 0 12.789 0zm0 1.75c-1.987 0-3.604 1.607-3.604 3.58a3.526 3.526 0 001.04 2.527 3.58 3.58 0 002.535 1.054l.03.875v-.875c1.987 0 3.605-1.605 3.605-3.58S14.777 1.75 12.789 1.75z" fill={color} fillOpacity={opacity} />
        </svg>
      );
    case "bell":
      return (
        <svg width={size} height={size} viewBox="0 0 20 22" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path fillRule="evenodd" clipRule="evenodd" d="M7.547 19.55c.533.59 1.218.915 1.93.915.714 0 1.403-.324 1.938-.916a.777.777 0 011.09-.056c.318.284.344.77.058 1.084-.832.917-1.927 1.423-3.086 1.423h-.002c-1.155-.001-2.248-.506-3.077-1.424a.762.762 0 01.057-1.083.774.774 0 011.092.057zM9.527 0c4.58 0 7.657 3.543 7.657 6.85 0 1.702.436 2.424.899 3.19.457.754.976 1.612.976 3.233-.36 4.14-4.713 4.478-9.531 4.478-4.818 0-9.172-.337-9.528-4.413-.003-1.686.515-2.544.973-3.299l.161-.27c.398-.679.737-1.417.737-2.918C1.871 3.543 4.948 0 9.528 0zm0 1.535c-3.6 0-6.11 2.802-6.11 5.316 0 2.127-.595 3.11-1.12 3.978-.422.697-.755 1.247-.755 2.444.173 1.93 1.455 2.944 7.986 2.944 6.494 0 7.817-1.06 7.988-3.01-.003-1.13-.336-1.681-.757-2.378-.526-.868-1.12-1.851-1.12-3.978 0-2.514-2.51-5.316-6.111-5.316z" fill={color} fillOpacity={opacity} />
        </svg>
      );
    case "chat":
      return (
        <svg width={size} height={size} viewBox="0 0 23 22" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path fillRule="evenodd" clipRule="evenodd" d="M11.43 0c2.96 0 5.743 1.143 7.833 3.22 4.32 4.29 4.32 11.271 0 15.562C17.145 20.886 14.293 22 11.405 22c-1.575 0-3.16-.33-4.643-1.012-.437-.174-.847-.338-1.14-.338-.338.002-.793.158-1.232.308-.9.307-2.022.69-2.852-.131-.826-.822-.445-1.932-.138-2.826.152-.44.307-.895.307-1.239 0-.282-.137-.642-.347-1.161C-.57 11.46.322 6.47 3.596 3.22A11.04 11.04 0 0111.43 0zm0 1.535A9.5 9.5 0 004.69 4.307a9.463 9.463 0 00-1.91 10.686c.241.592.474 1.17.474 1.77 0 .598-.207 1.201-.39 1.733-.15.439-.378 1.1-.231 1.245.143.147.813-.085 1.255-.235.53-.18 1.133-.387 1.73-.391.597 0 1.161.225 1.758.463 3.655 1.679 7.98.915 10.796-1.881 3.716-3.693 3.716-9.7 0-13.391a9.5 9.5 0 00-6.74-2.77zm4.068 8.867c.57 0 1.03.458 1.03 1.024 0 .566-.46 1.023-1.03 1.023a1.023 1.023 0 11-.01-2.047h.01zm-4.131 0c.568 0 1.03.458 1.03 1.024 0 .566-.462 1.023-1.03 1.023a1.03 1.03 0 01-1.035-1.024c0-.566.455-1.023 1.025-1.023h.01zm-4.132 0c.568 0 1.03.458 1.03 1.024 0 .566-.462 1.023-1.03 1.023a1.022 1.022 0 11-.01-2.047h.01z" fill={color} fillOpacity={opacity} />
        </svg>
      );
    case "explore":
      return (
        <svg width={size} height={size} viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M10 0c5.523 0 10 4.477 10 10s-4.477 10-10 10S0 15.523 0 10 4.477 0 10 0zm0 1.395a8.605 8.605 0 100 17.21 8.605 8.605 0 000-17.21zm-1.233 4.65l.104.01c.188.028.443.113.668.203 1.026.398 3.033 1.746 3.8 2.563l.223.239.08.092a1.16 1.16 0 01.025 1.405c-.04.053-.086.105-.19.215l-.269.28c-.812.794-2.57 1.971-3.569 2.391-.277.117-.675.25-.865.253a1.167 1.167 0 01-1.07-.629c-.053-.104-.12-.353-.171-.586l-.051-.262c-.093-.57-.143-1.437-.142-2.347l.001-.288c.01-.858.063-1.64.157-2.147.037-.207.12-.563.167-.678.104-.25.291-.45.523-.575a1.15 1.15 0 01.58-.14zm.14 1.467l-.027.126-.034.198c-.07.483-.112 1.233-.111 2.036l.001.279c.009.737.053 1.414.123 1.841l.048.235.192-.07c.883-.372 2.636-1.56 3.23-2.2l.08-.087-.212-.218c-.711-.682-2.38-1.79-3.167-2.095l-.124-.045z" fill={color} />
        </svg>
      );
    case "chevron-down":
      return (
        <svg width={size} height={size} viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M5 5l.354.354L5 5.707l-.354-.353L5 5zm4.354-3.646l-4 4-.708-.708 4-4 .708.708zm-4.708 4l-4-4 .708-.708 4 4-.708.708z" fill={color} />
        </svg>
      );
    case "more-vertical":
      return (
        <svg width={size} height={size} viewBox="0 0 4 17" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <circle cx="2" cy="2" r="2" fill={color} />
          <circle cx="2" cy="8" r="2" fill={color} />
          <circle cx="2" cy="14" r="2" fill={color} />
        </svg>
      );
    case "search":
      return (
        <svg width={size} height={size} viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <circle cx="7" cy="7" r="6" stroke={color} strokeWidth="1.5" />
          <path d="M16 16l-3-3" stroke={color} strokeWidth="1.5" strokeLinecap="round" />
        </svg>
      );
    case "settings":
      return (
        <svg width={size} height={size} viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M9.584 0c.671 0 1.315.267 1.783.74.468.473.721 1.112.7 1.709l.009.14a.985.985 0 00.136.395c.145.242.382.418.659.488.276.071.57.03.849-.13l.155-.078c1.165-.538 2.563-.11 3.21.991l.58.99a.695.695 0 01.04.081l.055.107c.519 1.089.15 2.385-.838 3.043l-.244.15a1.046 1.046 0 00-.313.339 1.042 1.042 0 00-.11.805c.074.272.255.504.53.66l.158.1c.478.328.823.812.973 1.367.17.626.08 1.292-.257 1.86l-.625 1.022-.094.144c-.735 1.038-2.16 1.355-3.248.738l-.129-.066a1.123 1.123 0 00-.412-.095 1.087 1.087 0 00-.766.31c-.204.2-.317.471-.316.786l-.008.163C11.956 18.022 10.88 19 9.584 19h-1.17c-1.373 0-2.486-1.093-2.484-2.398l-.008-.14a.994.994 0 00-.14-.401 1.066 1.066 0 00-.652-.493 1.12 1.12 0 00-.852.127l-.169.083a2.526 2.526 0 01-1.698.122 2.47 2.47 0 01-1.488-1.154l-.604-1.024-.08-.152a2.404 2.404 0 01.975-3.132l.1-.061c.292-.199.467-.527.467-.877 0-.381-.207-.733-.569-.94l-.147-.092a2.419 2.419 0 01-.724-3.236l.615-.993a2.503 2.503 0 013.366-.912l.126.066c.13.058.269.089.403.09a1.08 1.08 0 001.086-1.068l.008-.185c.049-.57.301-1.106.713-1.513A2.5 2.5 0 018.414 0h1.17zM9.584 6.396a3.105 3.105 0 100 6.21 3.105 3.105 0 000-6.21z" fill={active ? color : "#377DFF"} />
        </svg>
      );
    case "help":
      return (
        <svg width={size} height={size} viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M10 19a9 9 0 100-18 9 9 0 000 18z" stroke={active ? color : "#377DFF"} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
          <path d="M7.38 7.3a2.7 2.7 0 015.248.9c0 1.8-2.7 2.7-2.7 2.7M10 14.5h.009" stroke={active ? color : "#377DFF"} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      );
    case "logout":
      return (
        <svg width={size} height={size} viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M11.25 6.75L15 10.5m0 0l-3.75 3.75M15 10.5H6.75m0-7.5h-1.5A2.25 2.25 0 003 5.25v10.5A2.25 2.25 0 005.25 18h1.5" stroke="#FF3737" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      );
    case "close":
      return (
        <svg width={size} height={size} viewBox="0 0 6 10" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M1.354 9.354l4-4-.708-.708-4 4 .708.708zm4-4.708l-4-4-.708.708 4 4 .708-.708z" fill="currentColor" />
        </svg>
      );
    case "like":
      return (
        <svg width={size} height={size} viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M9.5 19a9.5 9.5 0 100-19 9.5 9.5 0 000 19z" fill="#FFCC4D" />
          <path d="M9.5 11.083c-1.912 0-3.181-.222-4.75-.527-.358-.07-1.056 0-1.056 1.055 0 2.111 2.425 4.75 5.806 4.75 3.38 0 5.805-2.639 5.805-4.75 0-1.055-.697-1.125-1.055-1.055-1.57.305-2.838.527-4.75.527z" fill="#664500" />
          <path d="M4.75 11.611s1.583.528 4.75.528 4.75-.528 4.75-.528-1.056 2.111-4.75 2.111-4.75-2.11-4.75-2.11z" fill="#fff" />
          <path d="M6.333 8.972c.729 0 1.32-.827 1.32-1.847s-.591-1.847-1.32-1.847c-.729 0-1.32.827-1.32 1.847s.591 1.847 1.32 1.847zM12.667 8.972c.729 0 1.32-.827 1.32-1.847s-.591-1.847-1.32-1.847c-.729 0-1.32.827-1.32 1.847s.591 1.847 1.32 1.847z" fill="#664500" />
        </svg>
      );
    case "comment":
      return (
        <svg width={size} height={size} viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M1 10.5c0-.464 0-.696.009-.893A9 9 0 019.607 1.01C9.804 1 10.036 1 10.5 1v0c.464 0 .696 0 .893.009a9 9 0 018.598 8.598c.009.197.009.429.009.893v6.046c0 1.36 0 2.041-.317 2.535a2 2 0 01-.602.602c-.494.317-1.174.317-2.535.317H10.5c-.464 0-.696 0-.893-.009a9 9 0 01-8.598-8.598C1 11.196 1 10.964 1 10.5v0z" stroke={color} strokeWidth="1.5" />
          <path d="M6.938 9.313h7.125M10.5 14.063h3.563" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      );
    case "share":
      return (
        <svg width={size} height={size} viewBox="0 0 24 21" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M23 10.5L12.917 1v5.429C3.267 6.429 1 13.258 1 20c2.785-3.52 5.248-5.429 11.917-5.429V20L23 10.5z" stroke={color} strokeWidth="1.5" strokeLinejoin="round" />
        </svg>
      );
    case "plus":
      return (
        <svg width={size} height={size} viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M5 1v8M1 5h8" stroke={color} strokeWidth="2" strokeLinecap="round" />
        </svg>
      );
    case "photo-fill":
      return (
        <svg width={size} height={size} viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M13.916 0c3.109 0 5.18 2.429 5.18 5.914v8.17c0 3.486-2.072 5.916-5.18 5.916H5.999C2.89 20 .827 17.572.827 14.085v-8.17C.827 2.43 2.897 0 6 0h7.917zm0 1.504H5.999c-2.321 0-3.799 1.735-3.799 4.41v8.17c0 2.68 1.472 4.412 3.799 4.412h7.917c2.328 0 3.807-1.734 3.807-4.411v-8.17c0-2.678-1.478-4.411-3.807-4.411zm.65 8.68l.12.125 1.9 2.147a.803.803 0 01-.016 1.063.642.642 0 01-.894.058l-.076-.074-1.9-2.148a.806.806 0 00-1.205-.028l-.074.087-2.04 2.717c-.722.963-2.02 1.066-2.86.26l-.111-.116-.814-.91a.562.562 0 00-.793-.07l-.075.073-1.4 1.617a.645.645 0 01-.97.029.805.805 0 01-.09-.977l.064-.086 1.4-1.617c.736-.852 1.95-.897 2.734-.137l.114.12.81.905a.587.587 0 00.861.033l.07-.078 2.04-2.718c.81-1.08 2.27-1.19 3.205-.275zM6.831 4.64c1.265 0 2.292 1.125 2.292 2.51 0 1.386-1.027 2.511-2.292 2.511S4.54 8.537 4.54 7.152c0-1.386 1.026-2.51 2.291-2.51zm0 1.504c-.507 0-.918.451-.918 1.007 0 .555.411 1.006.918 1.006.507 0 .919-.451.919-1.006 0-.556-.412-1.007-.919-1.007z" fill={color} />
        </svg>
      );
    case "video-fill":
      return (
        <svg width={size} height={size} viewBox="0 0 22 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M11.485 4.5c2.213 0 3.753 1.534 3.917 3.784l2.418-1.082c1.047-.468 2.188.327 2.271 1.533l.005.141v6.64c0 1.237-1.103 2.093-2.155 1.72l-.121-.047-2.418-1.083c-.164 2.25-1.708 3.785-3.917 3.785H5.76c-2.343 0-3.932-1.72-3.932-4.188V8.688c0-2.47 1.589-4.188 3.932-4.188h5.726zm0 1.5H5.76C4.169 6 3.197 7.05 3.197 8.688v7.015c0 1.636.972 2.688 2.562 2.688h5.726c1.586 0 2.562-1.054 2.562-2.688v-.686-6.329c0-1.636-.973-2.688-2.562-2.688zM18.4 8.57l-.062.02-2.921 1.306v4.596l2.921 1.307c.165.073.343-.036.38-.215l.008-.07V8.876c0-.195-.16-.334-.326-.305z" fill={color} />
        </svg>
      );
    case "event-fill":
      return (
        <svg width={size} height={size} viewBox="0 0 22 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M14.371 2c.32 0 .585.262.627.603l.005.095v.788c2.598.195 4.188 2.033 4.18 5v8.488c0 3.145-1.786 5.026-4.656 5.026H7.395C4.53 22 2.74 20.087 2.74 16.904V8.486c0-2.966 1.596-4.804 4.187-5v-.788c0-.386.283-.698.633-.698zm0 1.448h-6.22V5.1c0 .385-.285.698-.637.698-.32 0-.585-.262-.628-.602l-.005-.096V3.448h-.062c-2.029 0-2.73 1.205-2.73 3.551v8.486h11v-8.486c0-2.345-.7-3.551-2.718-3.551zm-7.058 6.046c.323 0 .589.263.632.604l.006.094v1.205c0 .386-.286.698-.638.698-.323 0-.59-.263-.632-.604l-.006-.094V10.19c0-.385.285-.697.638-.697zm6.756-.041c.323 0 .59.262.633.604l.005.094v1.205c0 .385-.286.697-.638.697-.323 0-.59-.262-.633-.603l-.005-.095v-1.205c0-.385.286-.697.638-.697zm-3.37.041c.323 0 .59.263.632.604l.006.094v1.205c0 .386-.285.698-.638.698-.323 0-.59-.263-.632-.604l-.006-.094V10.19c0-.385.286-.697.638-.697zm0 2.946c.323 0 .59.262.632.603l.006.095v1.22c0 .386-.285.698-.638.698s-.638-.312-.638-.698v-1.22c0-.34.258-.621.583-.69l.055-.008zm3.37-.04c.323 0 .59.262.633.603l.005.095v1.22c0 .385-.286.698-.638.698-.323 0-.59-.262-.633-.603l-.005-.095v-1.22c0-.385.286-.697.638-.697zm-6.756.04c.323 0 .589.263.632.604l.006.094v1.22c0 .385-.286.698-.638.698-.323 0-.59-.263-.632-.604l-.006-.094v-1.22c0-.385.285-.697.638-.697z" fill={color} />
        </svg>
      );
    case "article-fill":
      return (
        <svg width={size} height={size} viewBox="0 0 18 20" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M12.49 0c2.92 0 4.665 1.92 4.693 5.132v9.659c0 3.257-1.75 5.209-4.693 5.209H5.434c-.377 0-.734-.032-1.07-.095l-.2-.041C2 19.371.74 17.555.74 14.791V5.209c0-.334.019-.654.055-.96C1.114 1.564 2.799 0 5.434 0h7.056zm-.008 1.457H5.434c-2.244 0-3.381 1.263-3.381 3.752v9.582c0 2.489 1.137 3.752 3.38 3.752h7.049c2.242 0 3.372-1.263 3.372-3.752V5.209c0-2.489-1.13-3.752-3.372-3.752zm-.239 12.053c.36 0 .652.324.652.724 0 .4-.292.724-.652.724H5.656c-.36 0-.652-.324-.652-.724 0-.4.293-.724.652-.724h6.587zm0-4.239a.643.643 0 01.632.339.806.806 0 010 .78.643.643 0 01-.632.339H5.656c-.334-.042-.587-.355-.587-.729s.253-.688.587-.729h6.587zM8.17 5.042c.335.041.588.355.588.729 0 .373-.253.687-.588.728H5.665c-.336-.041-.589-.355-.589-.728 0-.374.253-.688.589-.729H8.17z" fill={color} />
        </svg>
      );
    case "send-fill":
      return (
        <svg width={size} height={size} viewBox="0 0 14 13" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path fillRule="evenodd" clipRule="evenodd" d="M6.37 7.879l2.438 3.955a.335.335 0 00.34.162c.068-.01.23-.05.289-.247l3.049-10.297a.348.348 0 00-.09-.35.341.341 0 00-.34-.088L1.75 4.03a.34.34 0 00-.247.289.343.343 0 00.16.347L5.666 7.17 9.2 3.597a.5.5 0 01.712.703L6.37 7.88z" fill={color} />
        </svg>
      );
    case "bookmarks":
      return (
        <svg width={size} height={size} viewBox="0 0 22 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M13.704 2c2.8 0 4.585 1.435 4.585 4.258V20.33c0 .443-.157.867-.436 1.18-.279.313-.658.489-1.063.489a1.456 1.456 0 01-.708-.203l-5.132-3.134-5.112 3.14c-.615.36-1.361.194-1.829-.405l-.09-.126-.085-.155a1.913 1.913 0 01-.176-.786V6.434C3.658 3.5 5.404 2 8.243 2h5.46zm0 1.448h-5.46c-2.191 0-3.295.948-3.295 2.986V20.32c0 .4 0 .9.114.1.034.063.059.09.1.074l5.11-3.138c.38-.23.84-.23 1.222.001l5.124 3.128a.252.252 0 00.114.035.188.188 0 00.14-.064.236.236 0 00.058-.157V6.258c0-1.9-1.132-2.81-3.294-2.81zm.386 4.869c.357 0 .646.324.646.723 0 .367-.243.67-.559.718l-.087.006H7.81c-.357 0-.646-.324-.646-.723 0-.367.243-.67.558-.718l.088-.006h6.28z" fill={color} />
        </svg>
      );
    case "save":
      return (
        <svg width={size} height={size} viewBox="0 0 22 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          <path d="M17 21 17 13 7 13 7 21" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          <path d="M7 3 7 8 15 8" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      );
    case "gaming":
      return (
        <svg width={size} height={size} viewBox="0 0 22 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
          <path d="M7.625 2c.315-.015.642.306.645.69.003.309.234.558.515.558h.928c1.317 0 2.402 1.169 2.419 2.616v.24h2.604c2.911-.026 5.255 2.337 5.377 5.414.005.12.006.245.004.368v4.31c.062 3.108-2.21 5.704-5.064 5.773-.117.003-.228 0-.34-.005a199.325 199.325 0 01-7.516 0c-2.816.132-5.238-2.292-5.363-5.411a6.262 6.262 0 01-.004-.371V11.87c-.03-1.497.48-2.931 1.438-4.024.956-1.094 2.245-1.714 3.629-1.746a3.28 3.28 0 01.342.005l3.617-.001v-.231c-.008-.676-.522-1.23-1.147-1.23h-.93c-.973 0-1.774-.866-1.785-1.937-.003-.386.28-.701.631-.705zm-.614 5.494h-.084C5.88 7.52 4.91 7.987 4.19 8.812c-.723.823-1.107 1.904-1.084 3.045v4.34c-.002.108 0 .202.003.294.094 2.353 1.903 4.193 4.07 4.08 2.487.046 5.013.046 7.55-.001.124.006.212.007.3.004 2.147-.05 3.86-2.007 3.812-4.361V11.87a5.027 5.027 0 00-.002-.291c-.093-2.338-1.82-4.082-4.029-4.082l-.07.002H7.209a4.032 4.032 0 00-.281-.004l.084-.001zm1.292 4.091c.341 0 .623.273.667.626l.007.098-.001 1.016h.946c.372 0 .673.325.673.725 0 .366-.253.669-.582.717l-.091.006h-.946v1.017c0 .4-.3.724-.673.724-.34 0-.622-.273-.667-.626l-.006-.098v-1.017h-.945c-.372 0-.674-.324-.674-.723 0-.367.254-.67.582-.718l.092-.006h.945v-1.017c0-.4.301-.724.673-.724zm7.058 3.428c.372 0 .674.324.674.724 0 .366-.254.67-.582.717l-.091.007h-.09c-.373 0-.674-.324-.674-.724 0-.367.253-.67.582-.717l.091-.007h.09zm-1.536-3.322c.372 0 .673.324.673.724 0 .367-.253.67-.582.718l-.091.006h-.09c-.372 0-.674-.324-.674-.724 0-.366.254-.67.582-.717l.092-.007h.09z" fill={color} />
        </svg>
      );
    default:
      return null;
  }
};
