...
const navItems = [
  {
    ...
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="27" fill="none" viewBox="0 0 24 27">
        <path fill="#000" fillOpacity=".6" stroke="#666" strokeWidth="1.5" d="M1 13.042c0-2.094 0-3.141.431-4.061.432-.92 1.242-1.602 2.862-2.965l1.571-1.321C8.792 2.232 10.256 1 12 1c1.744 0 3.208 1.232 6.136 3.695l1.572 1.321c1.62 1.363 2.43 2.044 2.86 2.965.432.92.432 1.967.432 4.06v6.54c0 2.908 0 4.362-.92 5.265-.921.904-2.403.904-5.366.904H7.286c-2.963 0-4.445 0-5.365-.904C1 23.944 1 22.49 1 19.581v-6.54z" />
  ...
  },
  {
    ...
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="27" height="20" fill="none" viewBox="0 0 27 20">
        <path fill="#000" fillOpacity=".6" fillRule="evenodd" d="M13.334 12.405h.138l.31.001c2.364.015 7.768.247 7.768 3.81 0 3.538-5.215 3.769-7.732 3.784h-.932c-2.364-.015-7.77-.247-7.77-3.805 0-3.543 5.405-3.774 7.77-3.789l.31-.001h.138zM13.334 0c3.086 0 5.596 2.442 5.596 5.442 0 3.001-2.51 5.443-5.596 5.443H13.3a5.616 5.616 0 01-3.943-1.603A5.308 5.308 0 017.74 5.439C7.739 2.442 10.249 0 13.334 0z" clipRule="evenodd" />
      </svg>
    ),
  },
  {
  ...
  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-bs-border shadow-[0_-4px_12px_rgba(108,126,147,0.1)]">
      <div className="flex items-center justify-around px-2 py-2">
          ...
          return (
            <Link
              ...
              className={`relative flex flex-col items-center gap-0.5 px-3 py-1 rounded-[8px] transition-all ${
                isActive ? "text-bs-primary" : "text-bs-muted"
              }`}
            >
              ...
              <span className={`text-[9px] font-medium ${isActive ? "text-bs-primary" : "text-bs-muted"}`}>
                {item.label}
              </span>
            </Link>
          );
        ...
        {/* Menu Hamburger */}
        <button className="flex flex-col items-center gap-0.5 px-3 py-1 rounded-[8px] text-bs-muted hover:text-bs-primary transition-all">
          <svg xmlns="http://www.w3.org/2000/svg" width="22" height="18" fill="none" viewBox="0 0 18 14">
