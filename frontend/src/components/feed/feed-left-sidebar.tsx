  ...
  return (
    <aside className="hidden lg:flex flex-col gap-[16px] w-full">
      ...
      <div className="bg-white rounded-[6px] px-[24px] pt-[24px] pb-[16px] border border-bs-bg">
        <h4 className="text-bs-dark font-semibold text-[18px] mb-[24px] font-[Poppins]">Explore</h4>
        <ul className="space-y-[12px]">
          {exploreItems.map((item) => (
            <li key={item.label} className="mb-[24px]">
              <Link
                href={item.href ?? "#0"}
                className="flex items-center text-bs-muted text-[16px] font-[Poppins] hover:text-bs-primary leading-[1.4] font-medium transition-all group relative"
              >
                <div className="mr-[14px] flex items-center justify-center group-hover:text-bs-primary">
                  {item.icon}
                </div>
                {item.label}
                {item.badge && (
                  <span className="absolute right-0 text-white text-[13px] font-normal border-2 border-white rounded-[8px] w-[36px] h-[24px] flex items-center justify-center bg-green-500">
                    {item.badge}
                  </span>
                )}
              </Link>
      ...
      <div className="bg-white rounded-[6px] px-[24px] pt-[24px] pb-[16px] border border-bs-bg">
        <div className="flex items-center justify-between mb-[24px]">
          <h4 className="text-bs-dark font-semibold text-[18px] font-[Poppins]">Suggested People</h4>
            ...
            <div key={person.name} className="flex items-center justify-between">
              <div className="flex items-center flex-1">
                <Link href={person.href} className="mr-[16px]">
                  <img src={person.img} alt={person.name} className="w-[40px] h-[40px] rounded-full object-cover shrink-0" />
                </Link>
                <div className="flex-1">
                  <Link href={person.href}>
                    <h4 className="text-[14px] font-medium text-bs-dark font-[Poppins] leading-[1.1] hover:text-bs-primary transition-colors">{person.name}</h4>
                  </Link>
                  <p className="text-[11px] font-light text-bs-muted font-[Poppins] leading-[1.4] mt-[4px]">{person.role}</p>
                </div>
              </div>
              <a href="#0" className="text-[12px] text-[#959EAE] font-medium font-[Poppins] border border-[#DCDFE4] bg-white rounded-[2px] py-[7px] px-[7px] hover:text-[#F3F9FF] hover:bg-bs-primary hover:border-bs-primary transition-all">
                Connect
              </a>
            </div>
          ))}
      ...
      {/* ── Events Card ── */}
      <div className="bg-white rounded-[6px] mb-[16px] border border-bs-bg">
        <div className="flex items-center justify-between px-[24px] pt-[24px] mb-[24px]">
          <h4 className="text-bs-dark font-semibold text-[18px] font-[Poppins]">Events</h4>
          <Link href="#" className="text-[12px] font-medium text-bs-primary leading-[18px] font-[Poppins]">See all</Link>
        </div>
        <div className="px-[24px] pb-[24px]">
          {events.map((ev, i) => (
            <div key={i} className="mb-[16px] bg-white rounded-[6px] cursor-pointer border border-bs-bg">
              ...
              <div className="flex items-center p-[20px_16px_14px_16px]">
                ...
                <div className="pl-[8px] pt-[5px] flex-1">
                  <h4 className="text-[16px] font-medium text-black leading-[1.4] font-[Poppins]">{ev.title}</h4>
                </div>
              </div>
              <div className="bg-bs-bg h-px my-[4px] mb-[10px] w-full" />
              <div className="flex items-center justify-between p-[2px_16px_12px_16px]">
                <p className="text-[12px] font-medium text-[#8A8A8A] font-[Poppins] opacity-70 leading-[18px]">{ev.going}</p>
                <span className="text-[12px] font-medium text-bs-primary bg-[#F3F9FF] border border-bs-primary rounded-[2px] p-[3px_14px] leading-[18px] hover:bg-bs-primary hover:text-[#F3F9FF] transition-all font-[Poppins]">Going</span>
              </div>
            </div>
          ))}
        </div>
